...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        kalitwoline)
            ...
            ;;
        oneline)
	    PROMPT=$'%F{magenta}└─${tun0:+$tun0}[%B%F{blue}%(6~.%-1~/…/%4~.%5~)%b%F{magenta}]%F{reset} '
            ;;
        arch)
...
# START ARCH CONFIG VARIABLES
PROMPT_ALTERNATIVE=oneline
NEWLINE_BEFORE_PROMPT=yes
# STOP ARCH CONFIG VARIABLES
...
export PATH="$PNPM_HOME:$JAVA_HOME:$PATH:$(go env GOPATH)/bin:/home/user/.local/share/gem/ruby/3.0.0/bin"
